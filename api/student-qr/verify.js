...
function statusFromOut(out) {
  ...
  if (code === 'CONFIG_REQUIRED' || code === 'CALLER_AUTH_FAILED' || code === 'SERVER_ERROR' || code.startsWith('DB_')) return 500;
  return 400;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const out = await studentQrVerify(args);
    return send(res, statusFromOut(out), out);
  } catch (e) {
    return send(res, 400, { ok: false, error: { code: 'BAD_JSON', message: e?.message || 'verify 실패' } });
  }
}