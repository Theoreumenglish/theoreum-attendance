...
function statusFromOut(out) {
  ...
  if (code === 'CONFIG_REQUIRED' || code === 'CALLER_AUTH_FAILED' || code === 'SERVER_ERROR' || code === 'LOCK_TIMEOUT' || code.startsWith('DB_')) return 500;
  return 400;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return send(res, 400, { ok: false, error: { code: 'BAD_JSON', message: e?.message || 'staff verify 실패' } });
  }
}