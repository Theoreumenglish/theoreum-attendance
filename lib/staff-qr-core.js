...
function cfg() {
  return {
    ...
    qrHmacSecret: String(
      process.env.STAFF_QR_HMAC_SECRET ||
...
async function getStaffById(supabase, staffId) {
  ...
  if (snapErr) throw new Error(snapErr.message || 'staff_snapshot 조회 실패');
  if (snap) return snap;
  ...
  if (staffErr) throw new Error(staffErr.message || 'staff 조회 실패');
  return staff || null;
}
