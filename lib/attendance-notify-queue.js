...
function shouldDeferQueueRow(row, nowMs = Date.now()) {
  const action = String(row?.action_type || '').trim().toUpperCase();
  if (!action.startsWith('CLINIC_')) return false;
  const ms = parseIsoMs(row?.occurred_at || '');
  return Number.isFinite(ms) && ms > nowMs;
}

function clinicSkipReason(actionType, clinicTask = {}) {
  const action = String(actionType || '').trim().toUpperCase();
  const status = String(clinicTask?.status || '').trim().toUpperCase();
  if (!action.startsWith('CLINIC_')) return '';
  if (!clinicTask || !String(clinicTask.clinic_task_id || '').trim()) return 'CLINIC_TASK_NOT_FOUND';
  if (['DONE', 'PARTIAL', 'REJECTED', 'CANCELLED'].includes(status)) return 'CLINIC_ALREADY_HANDLED';
  if (action === 'CLINIC_ABSENCE_PARENT' && !['CANDIDATE', 'PENDING'].includes(status)) return 'CLINIC_ALREADY_STARTED';
  return '';
}
  ...
  if (!actionPrefix) {
    return {
      ...
      error: {
        ...
        message: '재처리 대상 action_prefix가 필요합니다. ABSENT, ATTENDANCE, CLINIC 중 하나를 사용하세요.'
      }
    };
  }
...
export async function runAttendanceNotifyWorker(args = {}) {
  ...
  for (const row of pending) {
    if (shouldDeferQueueRow(row, workerNowMs)) {
      summary.skipped++;
      summary.items.push({
    ...
    try {
      ...
      } else if (claimedAction.startsWith('CLINIC_')) {
        let clinicTask = {};
        const clinicTaskId = String(claimed.trace_id || '').trim();
        if (clinicTaskId) {
          const { data: clinicRow } = await supabase
            .from('clinic_tasks')
            .select('clinic_task_id, title, task_type, source_type, due_date, due_time, due_at, clinic_mode, parent_note, parent_visible, status')
            .eq('clinic_task_id', clinicTaskId)
            .maybeSingle();
          clinicTask = clinicRow || {};
        }
        const skipReason = clinicSkipReason(claimedAction, clinicTask);
        if (skipReason) {
          const { error: skipErr } = await supabase
            .from('attendance_notify_queue')
            .update({
              status: 'DONE',
              sent_channel: 'SKIPPED',
          ...
          if (skipErr) {
            summary.failed++;
            summary.items.push({ queue_id: claimed.queue_id, status: 'FAILED', error: skipErr.message || '클리닉 알림 skip 업데이트 실패' });
          } else {
            summary.skipped++;
            summary.items.push({ queue_id: claimed.queue_id, status: 'SKIPPED', error: skipReason });
          }
          continue;
        }
        notifyResult = await notifyParentOnClinicDirect(
          studentPayload,
          clinicTask,
          claimed.trace_id,
          claimedAction,
