...
export async function enqueueAttendanceNotify(student, actionType, traceId) {
  ...
  const { data: existing, error: existingErr } = await supabase
    ...
    .eq('trace_id', String(traceId || '').trim())
...
export async function runAttendanceNotifyWorker(args = {}) {
  ...
  const limit = toPositiveInt(
    args?.limit,
    toPositiveInt(process.env.ATT_NOTIFY_WORKER_BATCH, 10)
  );
  const maxAttempts = toPositiveInt(process.env.ATT_NOTIFY_MAX_ATTEMPTS, 3);
