...
export async function getAttendanceMetaCached(options = {}) {
  ...
  _metaInflight = (async () => {
    ...
    if (_metaCache) {
      const maxStaleMs = toPositiveInt(process.env.ATT_META_MAX_STALE_MS, 60000, 1000, 600000);
      const staleAgeMs =
        _metaCacheFetchedAt > 0
          ? Math.max(0, nowMs() - _metaCacheFetchedAt)
