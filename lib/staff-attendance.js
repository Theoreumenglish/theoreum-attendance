...
const ALLOWED_INPUT_MODES = new Set(['QR', 'WEB', 'MANUAL', 'PIN']);
...
export async function recalcStaffClockRollupsByDate(staffIdInput, yyyymmddInput) {
  const supabase = getSupabaseAdmin();
  const staffId = normalizeStaffId(staffIdInput);
  const yyyymmdd = String(yyyymmddInput || '').trim();

  if (!staffId || !/^\d{8}$/.test(yyyymmdd)) {
    return {
      ok: false,
      error: 'INVALID_ROLLUP_INPUT',
      detail: 'staff_id와 yyyymmdd가 필요합니다.'
    };
  }
  ...
  if (dayErr) {
    return {
      ...
      detail: dayErr.message || 'staff_clock_logs 일별 조회 실패'
    };
  }
  ...
  if (dailyErr) {
    return {
      ...
      detail: dailyErr.message || 'staff_daily 복구 실패'
    };
  }
  ...
  if (monthlyErr) {
    return {
      ok: false,
      error: 'DB_UPSERT_FAILED',
      detail: monthlyErr.message || 'staff_monthly 복구 실패'
    };
  }
...
async function recalcRollupsFromClockRecord(supabase, record) {
  ...
  if (!staffId || !ts) {
    return {
      ok: false,
      error: 'INVALID_CLOCK_RECORD',
      detail: 'staff_id 또는 ts가 올바르지 않습니다.'
    };
  }

  return recalcStaffClockRollupsByDate(staffId, kstYmdFromIso(ts));
}

function summarizeClockRollup(rollup) {
  if (!rollup) {
    return {
      ok: false,
      error: 'ROLLUP_NOT_RUN',
      detail: ''
    };
  }
  ...
  return {
    ok: !!rollup.ok,
    yyyymmdd: rollup.yyyymmdd || '',
    yyyymm: rollup.yyyymm || '',
    daily: rollup.daily || null,
    monthly: rollup.monthly || null,
    error: rollup.ok ? '' : String(rollup.error || ''),
    detail: rollup.ok ? '' : String(rollup.detail || '')
  };
}
...
export async function writeStaffClockAndRollup(payload, options = {}) {
  ...
  if (existing) {
    const rollup = await recalcRollupsFromClockRecord(supabase, existing);
    ...
    return {
      ...
      duplicate: true,
      record: existing,
      rollup: summarizeClockRollup(rollup)
    };
  }
  ...
  if (dedupeSec > 0) {
    ...
    if (dup) {
      const rollup = await recalcRollupsFromClockRecord(supabase, dup);
