...
export async function runAbsenceDetectionDirect(args = {}) {
  ...
  const checkedInRows = await selectInChunks(
    supabase,
    'attendance_logs',
    'student_id, action_type, result',
    'student_id',
    activeStudentIds,
    query => query
      .eq('yyyymmdd', yyyymmdd)
  ...
  const checkedInSet = new Set(checkedInRows.map(row => normalizeStudentId(row.student_id)).filter(Boolean));
  summary.alreadyCheckedInCount = checkedInSet.size;
  ...
  for (const relation of relations) {
    ...
    if (checkedInSet.has(sid)) continue;
