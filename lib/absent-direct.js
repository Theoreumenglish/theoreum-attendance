...
function parseStages(raw) {
  const source = Array.isArray(raw)
    ? raw
    : String(raw || process.env.ABSENT_STAGE_MINUTES || '5,20').split(/[,,/|;\s]+/);

  const out = source
    .map(x => Number(x))
    .filter(n => Number.isFinite(n) && n > 0 && n <= 240 && n % 5 === 0)
    .map(n => Math.floor(n));

  const unique = Array.from(new Set(out)).sort((a, b) => a - b);
  return unique.length >= 2 ? unique.slice(0, 2) : DEFAULT_STAGES;
}

function parseClassAlertStages(raw, fallbackStages) {
  const fallback = Array.isArray(fallbackStages) && fallbackStages.length >= 2
    ? fallbackStages.slice(0, 2)
    : DEFAULT_STAGES;
...
function buildClassStageMap(classRows, fallbackStages) {
  const map = new Map();

  for (const row of classRows || []) {
    const classId = String(row.class_id || '').trim();
    if (!classId) continue;
    map.set(classId, parseClassAlertStages(row.alert_delay, fallbackStages));
  }

  return map;
}

function absentStageCatchupGraceMin() {
  return toPositiveInt(
    process.env.ABSENT_STAGE_CATCHUP_GRACE_MIN,
    15,
    0,
    240
  );
...
function isFutureExcuse(row, nowMs) {
  ...
  if (/^\d{10,13}$/.test(until)) {
    parsed = until.length === 10
      ? Number(until) * 1000
      : Number(until);
  } else {
    parsed = Date.parse(until);
  }

  // 잘못된 값은 영구 면제처럼 처리하지 않는다.
...
async function insertQueueRows(supabase, rows) {
  const result = {
    ...
    batchFallbackRowCount: 0,
    failed: []
  };

  const batchSize = Math.max(
    1,
    Math.min(200, Number(process.env.ABSENT_QUEUE_INSERT_BATCH_SIZE || 100) || 100)
  );
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    if (!batchError) {
      result.queuedCount += chunk.length;
      result.batchSuccessCount++;
      continue;
    }

    result.batchFallbackCount++;
    result.batchFallbackRowCount += chunk.length;

    // 배치 insert가 duplicate 하나 때문에 통째로 실패할 수 있으므로
    // 실패 배치만 row 단위로 재시도하여 기존 카운트/오류 분류 정확도를 유지한다.
...
export async function runAbsenceDetectionDirect(args = {}) {
  ...
  const fallbackStages = parseStages(args.stages);
  ...
  const catchupGraceMin = absentStageCatchupGraceMin();

  const summary = {
    yyyymmdd,
    checked_at: nowIso(now),
    stages: fallbackStages,
    classStagePolicyCount: 0,
    catchupGraceMin,
    dryRun,
  ...
  const scheduledClassIds = Array.from(
    new Set(scheduledRows.map(row => String(row.class_id || '').trim()).filter(Boolean))
  );

  const classAlertRows = scheduledClassIds.length
    ? await selectInChunks(
        supabase,
        'classes',
        'class_id, alert_delay',
        'class_id',
        scheduledClassIds
      )
    : [];

  const classStageMap = buildClassStageMap(classAlertRows, fallbackStages);
  summary.classStagePolicyCount = classStageMap.size;

  const evaluatedSchedules = scheduledRows.map(row => {
    const classId = String(row.class_id || '').trim();
    const scheduleStages = classStageMap.get(classId) || fallbackStages;
    const startMs = parseKstStartMs(yyyymmdd, row.start);
    const lateMin = startMs ? Math.floor((nowMs - startMs) / 60000) : -1;
    const stageCandidates = scheduleStages.filter(stage => lateMin >= stage);
    const dueStages = stageCandidates.filter(stage => {
      const nextStage = scheduleStages.find(candidate => candidate > stage);
      const graceEnd = stage + catchupGraceMin;
      const hardEnd = nextStage ? Math.min(nextStage, graceEnd) : graceEnd;
      return lateMin < hardEnd;
    });
    const expiredStageSkippedCount = Math.max(0, stageCandidates.length - dueStages.length);
    ...
    return {
      ...
      lateMin,
      dueStages,
      expiredStageSkippedCount
    };
  });

  summary.expiredStageSkippedCount = evaluatedSchedules.reduce(
    (sum, row) => sum + (Number(row.expiredStageSkippedCount || 0) || 0),
    0
  );

  const lateSchedules = evaluatedSchedules
  ...
  for (const relation of relations) {
    ...
    for (const stage of schedule.dueStages) {
      ...
      candidates.push({
        ...
        stagePolicy: Array.isArray(schedule.stages) ? schedule.stages.join(',') : '',
        student
      });
    }
  }
  ...
  summary.batchCount = insertResult.batchCount || 0;
  summary.batchSuccessCount = insertResult.batchSuccessCount || 0;
  summary.batchFallbackCount = insertResult.batchFallbackCount || 0;
  summary.batchFallbackRowCount = insertResult.batchFallbackRowCount || 0;
