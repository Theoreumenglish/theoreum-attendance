...
function cfg() {
  return {
    stepMs: positiveInt(process.env.STUDENT_QR_FRAME_STEP_MS, 6000, 2000, 30000),
    graceMs: positiveInt(process.env.STUDENT_QR_GRACE_MS, 15000, 1000, 30000),
    sessionTtlSec: positiveInt(process.env.STUDENT_QR_SESSION_TTL_SEC, 90, 30, 600),
...
function normalizeStudentStatus(raw) {
  ...
  if (s === '재원' || s === 'active' || s === '재원생') return 'ACTIVE';
...
function hmacShortB64Url(msg) {
  return hmacB64Url(msg).slice(0, 16);
}

function hmacTinyB64Url(msg) {
  return hmacB64Url(msg).slice(0, 14);
}

function toBase36Ms(ms) {
  return Math.max(0, Number(ms || 0)).toString(36);
}

function fromBase36Ms(value) {
  const n = parseInt(String(value || '').trim(), 36);
...
async function closeStudentQrSessionAfterUse(supabase, sid, publicSessionId) {
  const studentId = normalizeSid(sid);
  const sessionId = String(publicSessionId || '').trim();

  if (!isSid(studentId) || !sessionId) {
    return {
      ok: false,
      error: 'INVALID_SESSION_CLOSE_INPUT'
    };
  }

  const { error: nonceDeleteErr } = await supabase
    .from('student_qr_nonces')
    .delete()
  ...
  if (nonceDeleteErr) {
    return {
      ok: false,
      error: nonceDeleteErr.message || 'unused nonce cleanup 실패'
    };
  }
  ...
  if (sessionDeleteErr) {
    return {
      ok: false,
      error: sessionDeleteErr.message || 'student QR session cleanup 실패'
    };
  }
...
export async function studentQrSessionStart(args) {
  ...
  const publicSessionId = randomB64Url(6);
  const now = nowMs();
...
export async function studentQrSessionFrame(args) {
  ...
  const { data: cachedNonce, error: cachedErr } = await supabase
    ...
    .eq('public_session_id', session.public_session_id)
    .eq('used', false)
    .eq('exp_ms', frameExp)
    .limit(1)
    .maybeSingle();
  ...
  if (cachedErr) {
    return err('DB_SELECT_FAILED', cachedErr.message || 'student_qr_nonces 조회 실패');
  }

  if (cachedNonce && cachedNonce.nonce) {
    nonce = String(cachedNonce.nonce).trim();
  } else {
    nonce = randomB64Url(6);
  ...
  const qrBody = [
    ...
    String(session.public_session_id || '').trim(),
    String(nonce || '').trim(),
    toBase36Ms(frameExp)
  ].join('.');

  const qrText = qrBody + '.' + hmacTinyB64Url(qrBody);
...
export async function studentQrVerify(args) {
  ...
  if (/^Q3\./i.test(token)) {
    ...
    if (parts.length !== 6 || String(parts[0] || '').toUpperCase() !== 'Q3') {
      return err('BAD_FORMAT', 'QR 포맷 오류');
    }

    const qrBody = parts.slice(0, 5).join('.');
    const sigB64 = String(parts[5] || '').trim();
    const expect = hmacTinyB64Url(qrBody);

    if (!secureEqual(sigB64, expect)) {
      return err('BAD_SIG', '서명 불일치');
    }

    sid = normalizeSid(parts[1]);
    publicSessionId = String(parts[2] || '').trim();
    nonce = String(parts[3] || '').trim();
    frameExp = fromBase36Ms(parts[4]);
  } else if (/^QR2\./i.test(token)) {
    const parts = token.split('.');

    if (parts.length !== 6 || String(parts[0] || '').toUpperCase() !== 'QR2') {
      return err('BAD_FORMAT', 'QR 포맷 오류');
    }

    const qrBody = parts.slice(0, 5).join('.');
    const sigB64 = String(parts[5] || '').trim();
    const expect = hmacShortB64Url(qrBody);
    ...
    sid = normalizeSid(obj?.s);
    publicSessionId = String(obj?.u || '').trim();
    nonce = String(obj?.n || '').trim();
    frameExp = Number(obj?.e || 0);
  }
  ...
  const { data: currentSession, error: sessionReadErr } = await supabase
    .from('student_qr_sessions')
    .select('student_id, public_session_id, exp_ms')
    .eq('student_id', sid)
    .eq('public_session_id', publicSessionId)
    .maybeSingle();

  if (sessionReadErr) {
    return err('DB_SELECT_FAILED', sessionReadErr.message || 'student_qr_sessions 조회 실패');
  }
  if (!currentSession) {
    return err('SESSION_EXPIRED', '이미 새 세션으로 교체된 QR입니다.');
  }
  if (Number(currentSession.exp_ms || 0) < now) {
    return err('SESSION_EXPIRED', '이미 새 세션으로 교체된 QR입니다.');
  }

  if (!consume) {
    if (issued.used) return err('ALREADY_USED', '이미 사용된 QR');
  ...
  const sessionClose = await closeStudentQrSessionAfterUse(
    supabase,
    sid,
    publicSessionId
  );
  ...
  return ok({
    ...
    qr_id: String(student.qr_id || ''),
    consumed: true,
    session_closed: !!sessionClose.ok,
    session_close_error: sessionClose.ok ? '' : String(sessionClose.error || '')
  });
}
...
export async function studentQrSessionStop(args) {
  ...
  const { data: session, error: sessErr } = await supabase
    .from('student_qr_sessions')
    .select('token, student_id, public_session_id')
    .eq('token', token)
    .maybeSingle();

  if (sessErr) {
    return err('DB_SELECT_FAILED', sessErr.message || 'student_qr_sessions 조회 실패');
  }
  ...
  if (session) {
    await supabase
      .from('student_qr_nonces')
      .delete()
      .eq('student_id', String(session.student_id || '').trim())
