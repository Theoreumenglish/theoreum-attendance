...
function buildClinicTemplateText(student, clinicTask = {}) {
  const studentLine = [
    String(student?.school || '').trim(),
    String(student?.grade || '').trim(),
    String(student?.student_name || '').trim()
  ].filter(Boolean).join(' ');
  const title = String(clinicTask?.title || '클리닉').trim();
  const dueDate = String(clinicTask?.due_date || '').trim();
  const parentNote = String(clinicTask?.parent_note || '').trim();
  const line1 = studentLine ? `${studentLine} 학생의 클리닉 안내입니다.` : '클리닉 안내입니다.';
  const dueLine = dueDate ? `\n예정일: ${dueDate}` : '';
  const noteLine = parentNote ? `\n내용: ${parentNote}` : '';
  return '[더오름영어 클리닉 안내]\n' +
    '안녕하세요 더오름영어 입니다.\n' +
    line1 + '\n' +
    `클리닉: ${title}` +
...
export async function notifyParentOnClinicDirect(student, clinicTask = {}, traceId = '') {
  const to = normPhone(student?.parent_phone);
  if (!to) {
    return { attempted: false, ok: false, channel: '', error: '학부모 전화번호 없음', reason: 'NO_PARENT_PHONE' };
  }

  const msg = buildClinicTemplateText(student, clinicTask);
  const tpl = env('TPL_CLINIC_NOTICE') || env('TPL_CLINIC');
  let result = {
    attempted: true,
    ok: false,
    channel: '',
  ...
  if (tpl) {
    ...
    result.error = alim.error || '클리닉 알림톡 실패';
    if (!allowSmsAfterAlimFail) return result;
  } else {
    result.error = 'TPL_CLINIC_NOTICE 미설정';
