...
export async function authLoginDirect(args = {}) {
  ...
  if (!pepperOk) {
    return loginFail(500, 'CONFIG_REQUIRED', 'AUTH_PEPPER 또는 SYS_PEPPER가 필요합니다.');
  }

  const staffId = normalizeStaffId(args.staff_id || args.staffId || args.id || '');
  const password = String(args.password || args.pw || '').trim();
  if (!staffId || !password) {
    return loginFail(400, 'INVALID_INPUT', '아이디/비밀번호가 필요합니다.');
  }
  ...
  if (!staff) return loginFail(401, 'AUTH_FAIL', '아이디 또는 비밀번호를 확인해주세요.');
  ...
  if (!pwHash || !pwSalt) {
    return loginFail(500, 'STAFF_SCHEMA_ERROR', 'staff_snapshot 비밀번호 해시/salt가 비어 있습니다.');
  }

  const ok = hashWithSalt(password, pwSalt) === pwHash;
  if (!ok) {
    return loginFail(401, 'AUTH_FAIL', '아이디 또는 비밀번호를 확인해주세요.');
  }
  ...
  if (insErr) {
    return loginFail(500, 'DB_INSERT_FAILED', insErr.message || 'staff_sessions insert 실패');
  }
