...
const AUTH_SESSION_CACHE = new Map();

function authSessionCacheSec() {
  const n = Number(process.env.AUTH_SESSION_CACHE_SEC || 25);
  return Number.isFinite(n) ? Math.max(0, Math.min(120, Math.floor(n))) : 25;
}

function readAuthSessionCache(tokenHash) {
  const ttlSec = authSessionCacheSec();
  if (!ttlSec || !tokenHash) return null;
  const hit = AUTH_SESSION_CACHE.get(tokenHash);
  if (!hit) return null;
  if (Date.now() > hit.cache_expire_ms || Date.now() > hit.session_expire_ms) {
...
function writeAuthSessionCache(tokenHash, me, sessionExpireMs) {
  const ttlSec = authSessionCacheSec();
  if (!ttlSec || !tokenHash || !me || !me.loggedIn) return;
  AUTH_SESSION_CACHE.set(tokenHash, {
    me: { ...me },
    cache_expire_ms: Date.now() + ttlSec * 1000,
    session_expire_ms: Number.isFinite(sessionExpireMs) ? sessionExpireMs : Date.now() + ttlSec * 1000
  });
  if (AUTH_SESSION_CACHE.size > 200) {
    const now = Date.now();
    for (const [key, value] of AUTH_SESSION_CACHE.entries()) {
      if (now > value.cache_expire_ms || now > value.session_expire_ms) AUTH_SESSION_CACHE.delete(key);
    }
  }
}

function deleteAuthSessionCache(tokenHash) {
  if (tokenHash) AUTH_SESSION_CACHE.delete(tokenHash);
}
...
function isAuthDebugEnabled() {
  return String(process.env.AUTH_DEBUG_LOGIN || 'N').trim().toUpperCase() === 'Y';
}

function debugLoginFail(status, code, message, detail = {}) {
  if (!isAuthDebugEnabled()) {
    return loginFail(status, code, message);
  }
...
if (!staff) {
  if (!isAuthDebugEnabled()) {
    return loginFail(401, 'AUTH_FAIL', '아이디 또는 비밀번호를 확인해주세요.');
  }
...
export async function authMeDirect(rawToken, options = {}) {
  ...
  const cached = options.touch === false ? readAuthSessionCache(tokenHash) : null;
  ...
  writeAuthSessionCache(tokenHash, out, expMs);
  return out;
}
