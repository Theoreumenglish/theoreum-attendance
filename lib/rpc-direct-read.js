...
export async function assistantGetLogsDirect(args = {}, sessionToken = '') {
  ...
  const logs = rows.map(row => {
    const mapped = mapAttendLogRow(row);
    return {
      ...mapped,
      student_name: studentNameMap[mapped.student_id] || ''
    };
  });
...
export async function assistantGetLogByTraceDirect(args = {}, sessionToken = '') {
  ...
  const mapped = mapAttendLogRow(data);
  const studentNameMap = await readStudentNameMap(supabase, [mapped.student_id]);
